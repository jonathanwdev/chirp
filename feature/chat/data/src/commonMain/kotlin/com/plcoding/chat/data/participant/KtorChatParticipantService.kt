...
import com.plcoding.chat.data.dto.requests.ConfirmProfilePictureRequestDTO
...
class KtorChatParticipantService(
    ...
    override suspend fun searchParticipant(query: String): Result<ChatParticipant, DataError.Remote> {
        return httpClient
            .get<ChatParticipantResponseDTO>(
                route = "/participants",
                queryParams = mapOf(
                    "query" to query
                )
    ...
    override suspend fun getProfilePictureUploadUrl(mimeType: String): Result<ProfilePictureUploadUrls, DataError.Remote> {
        return httpClient.post<Unit, ProfilePictureUploadResponseDTO>(
    ...
    override suspend fun confirmProfilePictureUpload(publicUrl: String): EmptyResult<DataError.Remote> {
        return httpClient.post<ConfirmProfilePictureRequestDTO, Unit>(
            route = "/participants/confirm-profile-picture",
            body = ConfirmProfilePictureRequestDTO(
                publicUrl = publicUrl
            )
        )
    }
