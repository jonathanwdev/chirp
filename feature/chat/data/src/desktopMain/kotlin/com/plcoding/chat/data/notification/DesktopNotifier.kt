...
class DesktopNotifier(
    ...
    fun observeNewNotifications(): Flow<NotificationPayload> {
        return combine(
        ...
        ) { chatMessages, authInfo ->
            val currentUserId = authInfo?.user?.id
            if(chatMessages.senderId != currentUserId) {
                (chatMessages to currentUserId)
            } else null
        }
            .filterNotNull()
            ...
            .map { (message, currentUserId) ->
                ...
                val senderName = chatInfo?.chat?.participants?.find { it.userId == message.senderId }?.username
