...
object DateUtils {
    fun formatMessageTime(instant: Instant, clock: Clock = Clock.System): UiText {
        ...
        return when(messageDateTime.date) {
            todayDate -> UiText.Resource(
                Res.string.today_x, arrayOf(formattedTime)
            )
            yesterdayDate -> UiText.Resource(
                Res.string.yesterday_x, arrayOf(formattedTime)
            )
    ...
    fun formateDateSeparator(date: LocalDate, clock: Clock = Clock.System): UiText {
        val timezone = TimeZone.currentSystemDefault()
